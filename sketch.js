...
function generateQuestions() {
    ...
    // Hulpfunctie: rond af op hele euro's
    function berekenSamengesteld(beginKapitaal, rentePerJaar) {
        // rentePerJaar = array van percentages, bijv. [5, 5, 5] of [4, -2] of [6, -3, 5]
        let kapitaal = beginKapitaal;
        for (let r of rentePerJaar) {
            kapitaal = kapitaal * (1 + r / 100);
        }
        return Math.round(kapitaal);
    }

    let questionTypes = [

        // ===== OPTIE 1: Zelfde rente meerdere jaren =====
        () => {
            let beginKapitaal = floor(random([1, 2, 3, 4, 5, 6, 8, 10])) * 1000;
            let rente = floor(random([2, 3, 4, 5, 6, 8, 10]));
            let jaren = floor(random(2, 5));
            let renteArray = Array(jaren).fill(rente);
            let eindKapitaal = berekenSamengesteld(beginKapitaal, renteArray);
            return {
                text: `€${beginKapitaal} bij ${rente}%\n per jaar na\n ${jaren} jaar = ?`,
                answer: eindKapitaal
            };
        },

        // ===== OPTIE 1b: Zelfde rente, bereken winst =====
        () => {
            let beginKapitaal = floor(random([1, 2, 3, 4, 5])) * 1000;
            let rente = floor(random([3, 4, 5, 6, 8, 10]));
            let jaren = floor(random(2, 4));
            let renteArray = Array(jaren).fill(rente);
            let eindKapitaal = berekenSamengesteld(beginKapitaal, renteArray);
            let winst = eindKapitaal - beginKapitaal;
            return {
                text: `€${beginKapitaal} bij ${rente}%\n na ${jaren} jaar\n de winst = ?`,
                answer: winst
            };
        ...
        // ===== OPTIE 1c: Waardedaling zelfde percentage =====
        () => {
            let beginWaarde = floor(random([5, 8, 10, 12, 15, 20])) * 1000;
            let daling = floor(random([5, 8, 10, 12, 15]));
            let jaren = floor(random(2, 4));
            let renteArray = Array(jaren).fill(-daling);
            let eindWaarde = berekenSamengesteld(beginWaarde, renteArray);
            return {
                text: `€${beginWaarde} daalt\n ${daling}% per jaar\n na ${jaren} jaar = ?`,
                answer: eindWaarde
            };
        },

        // ===== OPTIE 2: Verschillende rente jaar 1 en jaar 2 =====
        () => {
            let beginKapitaal = floor(random([1, 2, 3, 4, 5, 6, 8, 10])) * 1000;
            let rente1 = floor(random([2, 3, 4, 5, 6, 8, 10]));
            let rente2 = floor(random([2, 3, 4, 5, 6, 8, 10]));
            // Zorg dat ze echt verschillend zijn
            while (rente2 === rente1) rente2 = floor(random([2, 3, 4, 5, 6, 8, 10]));
            let eindKapitaal = berekenSamengesteld(beginKapitaal, [rente1, rente2]);
            return {
                text: `€${beginKapitaal}\njaar 1: ${rente1}%,\n jaar 2: ${rente2}% = ?`,
                answer: eindKapitaal
            };
        ...
        // ===== OPTIE 2b: Verschillende rente 3 jaar =====
        () => {
            let beginKapitaal = floor(random([1, 2, 3, 4, 5])) * 1000;
            let rente1 = floor(random([3, 4, 5, 6, 8]));
            let rente2 = floor(random([2, 3, 4, 5, 6]));
            let rente3 = floor(random([4, 5, 6, 8, 10]));
            let eindKapitaal = berekenSamengesteld(beginKapitaal, [rente1, rente2, rente3]);
            return {
                text: `€${beginKapitaal}\n jaar 1: ${rente1}% \n jaar 2: ${rente2}% \n jaar3: ${rente3}% = ?`,
                answer: eindKapitaal
            };
        ...
        // ===== OPTIE 3: Mix positieve en negatieve rente (2 jaar) =====
        () => {
            let beginKapitaal = floor(random([2, 3, 4, 5, 6, 8, 10])) * 1000;
            let positief = floor(random([5, 6, 8, 10, 12, 15]));
            let negatief = floor(random([3, 4, 5, 6, 8, 10]));
            // Willekeurig: positief eerst of negatief eerst
            let volgorde = random() < 0.5 ? [positief, -negatief] : [-negatief, positief];
            let eindKapitaal = berekenSamengesteld(beginKapitaal, volgorde);
            let teken1 = volgorde[0] > 0 ? '+' : '';
            let teken2 = volgorde[1] > 0 ? '+' : '';
            return {
                text: `€${beginKapitaal}\njaar 1: ${teken1}${volgorde[0]}%,\n jaar 2: ${teken2}${volgorde[1]}% = ?`,
                answer: eindKapitaal
            };
        },

        // ===== OPTIE 3b: Mix positief én negatief over 3 jaar =====
        () => {
            let beginKapitaal = floor(random([2, 3, 4, 5, 6, 8])) * 1000;
            let r1 = floor(random([5, 6, 8, 10]));
            let r2 = -floor(random([3, 4, 5, 6, 8]));
            let r3 = floor(random([4, 5, 6, 8, 10]));
            let eindKapitaal = berekenSamengesteld(beginKapitaal, [r1, r2, r3]);
            let t2 = r2 > 0 ? '+' : '';
            return {
                text: `€${beginKapitaal}\n jaar 1:+${r1}% \n jaar 2: ${t2}${r2}% \n jaar3: +${r3}% = ?`,
                answer: eindKapitaal
    ...
    // Genereer 10 vragen met een goede mix van alle typen
    // Zorg voor minstens 2 vragen van elk hoofdtype
    let typeIndices = [0, 1, 2, 3, 4, 5, 6, 0, 3, 5]; // Vaste mix voor balans
    typeIndices = shuffle(typeIndices);

    for (let i = 0; i < 10; i++) {
        let q = questionTypes[typeIndices[i]]();
        questions.push(q);
